/* ═══════════════════════ window.NovaScene ══════════════════════
   NOVA · 3D AI CORE SCENE (Three.js, ES module)

   Minimalist-futuristic: near-monochrome body, one cyan accent.
   Public API:
...
export class NovaScene {
  constructor(container, callbacks = {}) {
    ...
    this.state = "idle";
    this.cur = { ...STATE_TARGETS.idle };
    this.streamFlicker = 0;
    ...
    this.cam = {
      ...
      lastInteraction: performance.now(), pinchDist: 0,
    };
    this.pointer = { x: 0, y: 0, tx: 0, ty: 0 };
    this.raycaster = new THREE.Raycaster();
    this.scratch = new THREE.Vector3(); // reused — no per-frame allocation
  ...
  init() {
    ...
    this.scene.add(new THREE.HemisphereLight(0x3a4250, 0x05070b, 1.15));
    const key = new THREE.DirectionalLight(0xdfe8f2, 1.5);
    key.position.set(4, 6, 5);
    this.scene.add(key);
    const fill = new THREE.DirectionalLight(0x2a3444, 0.6);
    fill.position.set(-5, -2, -4);
    this.scene.add(fill);

    this.coreLight = new THREE.PointLight(0x38d3ea, 1.6, 18, 1.6);
    this.scene.add(this.coreLight);
    this.accentLight = new THREE.PointLight(0x8f7bf0, 0.55, 14, 2);
  ...
  buildGrid() {
    const grid = new THREE.GridHelper(60, 60, 0x1a212c, 0x141a24);
    grid.position.y = -2.6;
  ...
  buildCore() {
    ...
    const mat = new THREE.MeshStandardMaterial({
      ...
      metalness: 0.35,
      roughness: 0.32,
      emissive: new THREE.Color(0x38d3ea),
    ...
    const wireMat = new THREE.MeshBasicMaterial({
      color: 0x38d3ea, wireframe: true, transparent: true, opacity: 0.35,
    });
  ...
  buildShell() {
    ...
    const mat = new THREE.MeshBasicMaterial({
      color: 0x5a6478, wireframe: true, transparent: true, opacity: 0.13,
    });
  ...
  buildRings() {
    ...
    for (const d of defs) {
      const mat = new THREE.MeshStandardMaterial({
        color: 0x39424f, metalness: 0.65, roughness: 0.35,
        emissive: new THREE.Color(0x38d3ea), emissiveIntensity: 0.14,
      });
  ...
  buildParticles() {
    ...
    const mono = new THREE.Color(0x6b7688);
    const cyan = new THREE.Color(0x38d3ea);
  ...
  buildNodes() {
    ...
    for (let i = 0; i < count; i++) {
      const mat = new THREE.MeshStandardMaterial({
        color: 0xd9e2ec, metalness: 0.5, roughness: 0.3,
        emissive: new THREE.Color(0x38d3ea), emissiveIntensity: 0.3,
      });
    ...
    this.nodeLinks = new THREE.LineSegments(
      linkGeo,
      new THREE.LineBasicMaterial({ color: 0x55617a, transparent: true, opacity: 0.3 }),
    );
  ...
  pulse(color = 0x38d3ea) {
    this.spawnPulse(color);
    this.spawnPulse(color);
    this.spawnPulse(color);
  }
  ...
  updatePulses(dt) {
    for (let i = this.pulses.length - 1; i >= 0; i--) {
      ...
      p.mesh.material.opacity = Math.max(0, 0.5 * (1 - p.life));
      if (p.life >= 1) {
  ...
  bindEvents() {
    ...
    add(el, "pointermove", onPointerMove);
    add(el, "pointerdown", onPointerDown);
    add(window, "pointerup", onPointerUp);
    add(el, "wheel", onWheel, { passive: false });
    add(el, "touchmove", onTouchMove, { passive: false });
    add(el, "touchend", onTouchEnd);
  ...
  handleClick(e) {
    ...
    if (nodeHits.length > 0) {
      const idx = nodeHits[0].object.userData.index;
      if (this.callbacks.onDataNodeClick) this.callbacks.onDataNodeClick(idx);
      this.spawnPulse(0x8f7bf0);
      return;
    }
    const coreHits = this.raycaster.intersectObject(this.coreMesh, false);
    if (coreHits.length > 0) {
      if (this.callbacks.onCoreClick) this.callbacks.onCoreClick();
      this.pulse();
    }
  }
  ...
  tick(dt) {
    ...
    for (const ring of this.rings) {
      const speed = ring.userData.speed * s.ringSpeed;
    ...
    const posAttr = this.nodeLinks.geometry.getAttribute("position");
    let li = 0;
    for (let i = 0; i < this.nodes.length; i++) {
      ...
      if (li < posAttr.count) {
        ...
        posAttr.setXYZ(li, 0, 0, 0); li++;
      }
      /* Link node → next node (sparse, every other) */
      if (i % 2 === 0 && li < posAttr.count - 1) {
        const pj = this.nodes[(i + 1) % this.nodes.length].position;
        posAttr.setXYZ(li, x, y, z); li++;
    ...
    const pAttr = this.particles.geometry.getAttribute("position");
    ...
    for (let i = 0; i < this.particleData.length; i++) {
      ...
      const shrink = 1 - flow * (0.55 + 0.35 * Math.sin(t * 2 + d.flow * 20));
      this.scratch.set(px, py, bz).multiplyScalar(shrink);
      pAttr.setXYZ(i, this.scratch.x, this.scratch.y, this.scratch.z);
    }
    ...
    if (this.waveTimer > waveInterval && this.pulses.length < 4) {
      this.waveTimer = 0;
      this.spawnPulse(this.state === "error" ? 0xf0716c : 0x38d3ea);
    }
